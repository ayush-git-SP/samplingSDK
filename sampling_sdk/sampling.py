...
def sampling_compare(df, test_df, train_df, column_name, color=None, col_type=None):
    ...
    for key in dframe1:
        lis.append(
            OrderedDict({"Attribute": key, "Population": dframe1[key], "TrainSample": train_summ_stat[key],
...
def cluster_sampling(df, column_name, cluster, lst, action,col_type):
    ...
    if action == 'submit':
        fig, table1 = sampling_compare(
            y.to_frame(), y_test.to_frame(), y_train.to_frame(), column_name, lst, col_type)
